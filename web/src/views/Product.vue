<template>
  <div class="product-view">
      ...
      <el-col :span="24">
        <el-card>
          ...
          >
            ...
            <el-table-column prop="location" label="产地" />
            <el-table-column prop="description" label="描述" />
            <el-table-column prop="price" label="价格（元/斤）" />
            <el-table-column prop="purchase" label="进货量（斤）" />
            <el-table-column prop="sale" label="销售量（斤）" />
            <el-table-column prop="quantity" label="库存（斤）" />
    ...
    <el-row>
      <el-col :span="24">
        <el-card>
          ...
          >
            <el-table-column prop="id" label="ID" />
            <el-table-column prop="product.name" label="货物名称" />
            <el-table-column prop="product.brand" label="品牌" />
            <el-table-column prop="quantity" label="销售数量（斤）" />
            <el-table-column prop="product.price" label="销售单价（元/斤）" />
            ...
            <el-table-column
              ...
              :filter-method="(value, row) => row.payment == value"
            />
            <el-table-column prop="note" label="备注" />
            <el-table-column label="操作">
              <template #header>
      ...
      <el-col :span="24">
        <el-card>
          <template #header>
            <div class="card-header">
              ...
              <div style="display: inline-flex; align-items: center">
                <el-button plain type="primary" @click="handlePurchase"
                  >进货</el-button
                >
                <el-button
                  plain
                  type="primary"
                  @click="() => (is_table_show = !is_table_show)"
                  >切换显示</el-button
                >
              </div>
            </div>
          ...
          >
            ...
            <el-table-column prop="product.name" label="货物名称" />
            <el-table-column prop="product.brand" label="品牌" />
            <el-table-column prop="product.location" label="产地" />
            <el-table-column prop="supplier" label="供货单位" />
            <el-table-column prop="quantity" label="采购数量（斤）" />
            <el-table-column prop="unit_price" label="采购单价（元/斤）" />
            <el-table-column prop="total_price" label="金额" />
            <el-table-column prop="paid" label="已付金额" />
            <el-table-column prop="debt" label="欠款" />
            <el-table-column prop="note" label="备注" />
            <el-table-column label="操作">
              <template #header>
              ...
              <template #default="{ row }">
                <el-button
                  ...
                  type="primary"
                  :disabled="row.recall"
                  @click="handleRecallPurchase(row)"
                  >退货</el-button
                >
              </template>
            </el-table-column>
    ...
    <el-dialog
      ...
      :title="dialogVisible.edit ? '编辑商品' : '添加商品'"
    >
      <el-form
        ref="product"
        :model="product"
      ...
      >
        ...
        <el-form-item prop="location" label="产地">
          <el-input v-model="product.location" />
        </el-form-item>
    ...
    <el-dialog v-model="dialogVisible.purchase" title="进货">
      ...
      >
        ...
        <el-form-item prop="supplier" label="供货单位">
          <el-input v-model="purchase.supplier" />
        </el-form-item>
        <el-form-item prop="unit_price" label="采购单价">
          <el-input-number
            v-model.number="purchase.unit_price"
            :min="0"
            :step="0.1"
            :precision="2"
          />
        ...
        <el-form-item prop="total_price" label="总计金额">
          <el-input-number
            v-model.number="purchase.total_price"
            :min="0"
            :precision="2"
          />
        </el-form-item>
        ...
        <el-form-item prop="total_price" label="欠款">
          <el-input-number
            v-model.number="purchase.debt"
            :min="0"
            :precision="2"
          />
        </el-form-item>
        <el-form-item prop=" note" label="备注">
          <el-input v-model="purchase.note" type="textarea" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="submitForm('purchase')"
            >Save</el-button
          >
        </el-form-item>
      </el-form>
    ...
    filteredProductList() {
      return this.productList.filter((product) => {
        return (
          product.name
            .toLowerCase()
            .includes(this.search.product.toLowerCase()) ||
          product.description
            .toLowerCase()
            .includes(this.search.product.toLowerCase()) ||
    ...
    filteredOrderList() {
      return this.orderList.filter((order) => {
        return (
          order.product.name
            .toLowerCase()
            .includes(this.search.order.toLowerCase()) ||
      ...
      return this.purchaseList.filter((purchase) => {
        return (
          purchase.product.name
            .toLowerCase()
            .includes(this.search.purchase.toLowerCase()) ||
    ...
    getProductSummary(param) {
      ...
      columns.forEach((column, index) => {
        switch (index) {
          ...
          case 8:
            sums[index] = data
              .reduce((sum, item) => sum + item[column.property], 0)
              .toFixed(2)
            return
        }
      })
